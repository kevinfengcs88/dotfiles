...
2. **Tool-managed** — reinstalled by `install.sh`, never symlinked: npm globals,
   `rtk` (+ `RTK.md` / `rtk-rewrite.sh` / `.rtk-hook.sha256`,
...
**headroom was also removed** (the context-compressor proxy capped the model
context window at 200k and triggered spurious compaction); `install.sh` tears
both down on every machine it runs on.
